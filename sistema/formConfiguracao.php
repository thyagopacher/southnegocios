        ...
        <div class="box-body">
            <div class="row">
                <form id="fconfiguracao" name="fconfiguracao" method="post">
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            <label for="sexo">Status Padrão Proposta</label>
                            <select class="form-control" name="statuspadraoproposta" id="statuspadraoproposta">
                                <?php
                                    $resstatus = $conexao->comando("select codstatus, nome from statusproposta order by nome");
                                    $qtdstatus = $conexao->qtdResultado($resstatus);
                                    if ($qtdstatus > 0) {
                                        echo '<option value="">--Selecione--</option>';
                                        while ($status = $conexao->resultadoArray($resstatus)) {
                                            if (isset($configuracao["statuspadraoproposta"]) && $configuracao["statuspadraoproposta"] == $status["codstatus"]) {
