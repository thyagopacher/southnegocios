                ...
                <form target="_blank" id="fpcomissao" name="fpcomissao" method="post" onsubmit="return false;">
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <select class="form-control" name="codtabela" id="codtabela">
                                <?php
                                $restabela = $conexao->comando('select codtabela, nome from tabela where codempresa = ' . $_SESSION["codempresa"] . ' order by nome');
                                $qtdtabela = $conexao->qtdResultado($restabela);
                                if ($qtdtabela > 0) {
                                    echo '<option value="">--Selecione--</option>';
                                    while ($tabela = $conexao->resultadoArray($restabela)) {
                                        echo '<option value="', $tabela["codpessoa"], '">', $tabela["nome"], '</option>';
                                    }
                                } else {
                                    echo '<option value="">--Nada encontrado--</option>';
                                }
