...
if(isset($_SESSION["nome"])){
    ...
    $conexao = new Conexao();
    $codmanual = filter_input(INPUT_GET, 'codmanual', FILTER_VALIDATE_INT);
    if (isset($codmanual)) {
        $manual = $conexao->comandoArray("select * from manual where codmanual = '{$codmanual}'");
        $titulo = "Alterar manuals";
        $action = "../control/AtualizarManual.php";
    } else {
        $titulo = "Cadastrar manuals";
        $action = "../control/InserirManual.php";
    }
    $nivelp = $conexao->comandoArray("select * from nivelpagina where codnivel = '{$_SESSION["codnivel"]}' and codpagina = '87'");
}
?>
<!DOCTYPE html>
<html lang="pt-br">
    ...
    <body>
        ...
        <h3><?= $titulo ?></h3>
         <div id="tabs" style="width: 955px; margin: 0 auto;  margin-top: 55px;">
            ...
            <div id="tabs-1"><?php include("formManual.php"); ?></div>
            <div id="tabs-2"><?php include("formProcurarManual.php"); ?></div>
        ...
        <script src="./recursos/js/ajax/Manual.js"></script>
