...
if(isset($_SESSION["nome"])){
    ...
    $conexao = new Conexao();
    $codorgao = filter_input(INPUT_GET, 'codorgao', FILTER_VALIDATE_INT);
    if (isset($codorgao)) {
        $orgao = $conexao->comandoArray("select * from orgaopagador where codorgao = '{$codorgao}'");
        $titulo = "Alterar órgão pagador";
        $action = "../control/AtualizarOrgaoPagador.php";
    } else {
        $titulo = "Cadastrar órgão pagador";
        $action = "../control/InserirOrgaoPagador.php";
    }
    $nivelp = $conexao->comandoArray("select * from nivelpagina where codnivel = '{$_SESSION["codnivel"]}' and codpagina = '74'");
}
?>
<!DOCTYPE html>
<html lang="pt-br">
...
<script type="text/javascript" src="/visao/recursos/js/jquery-1.10.2.min.js"></script>
        ...
        <script src="./recursos/js/ajax/OrgaoPagador.js"></script>
