    ...
    $.ajax({
        ...
        success: function (data, textStatus, jqXHR) {
            if (data.situacao === true) {
                swal("Pessoa cadastrada", data.mensagem, "success");
...
function atualizar() {
    $.ajax({
        ...
        success: function (data, textStatus, jqXHR) {
            if (data.situacao === true) {
                swal("Pessoa atualizada", data.mensagem, "success");
                procurarPessoa2(true);
            } else if (data.situacao === false) {
    ...
    if (window.confirm("Deseja realmente excluir esse pessoa?")) {
        if (document.getElementById("codpessoa").value !== null && document.getElementById("codpessoa").value !== "") {
            $.ajax({
                ...
                type: "POST",
                data: {codpessoa: document.getElementById("codpessoa").value},
                dataType: 'json',
                success: function (data, textStatus, jqXHR) {
                    if (data.situacao === true) {
