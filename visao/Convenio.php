...
if (isset($_SESSION["nome"])) {
    ...
    $conexao = new Conexao();
    $codconvenio = filter_input(INPUT_GET, 'codconvenio', FILTER_VALIDATE_INT);
    if (isset($codconvenio)) {
        $convenio = $conexao->comandoArray("select * from convenio where codconvenio = '{$codconvenio}'");
        $titulo = "Alterar convenios";
        $action = "../control/AtualizarConvenio.php";
    } else {
        $titulo = "Cadastrar convenios";
        $action = "../control/InserirConvenio.php";
    }
    $nivelp = $conexao->comandoArray("select * from nivelpagina where codnivel = '{$_SESSION["codnivel"]}' and codpagina = '75'");
}
?>
<!DOCTYPE html>
<html lang="pt-br">
    ...
    <body>
        ...
        <script src="./recursos/js/ajax/Convenio.js"></script>
