...
if (!isset($_SESSION["nome"])) {
    ...
    if (isset($_GET["codramal"])) {
        $sql = "select ramal.*
        from ramal
        where codramal = '{$_GET["codramal"]}' and codempresa = '{$_SESSION['codempresa']}'";
        $ramal = $conexao->comandoArray($sql);
        $titulo = "Alterar um telefone útil";
        $action = "../control/AtualizarRamal.php";
    } else {
        $titulo = "Cadastrar um telefone útil";
        $action = "../control/InserirRamal.php";
    }
    $nivelp = $conexao->comandoArray("select * from nivelpagina where codnivel = '{$_SESSION["codnivel"]}' and codpagina = '39'");
}
?>
<!DOCTYPE html>
...
<html lang="pt-br">
    ...
    <body>
        ...
        <script type="text/javascript" src="./recursos/js/ajax/Ramal.js"></script>
