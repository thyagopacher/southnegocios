...
if(isset($_SESSION["nome"])){
    ...
    $conexao = new Conexao();
    $codfrase = filter_input(INPUT_GET, 'codfrase', FILTER_VALIDATE_INT);
    if (isset($codfrase)) {
        $sql = "select * from frase where codfrase = '{$codfrase}'";
        $fraseForm = $conexao->comandoArray($sql);
        $titulo = "Alterar frases";
        $action = "../control/AtualizarFrase.php";
    } else {
        $titulo = "Cadastrar frases";
        $action = "../control/InserirFrase.php";
    }
    $nivelp = $conexao->comandoArray("select * from nivelpagina where codnivel = '{$_SESSION["codnivel"]}' and codpagina = '79'");
}
?>
<!DOCTYPE html>
<html lang="pt-br">
    ...
    <body>
        ...
        <h3><?= $titulo ?></h3>
         <div id="tabs" style="width: 955px; margin: 0 auto;  margin-top: 55px;">
            ...
            <div id="tabs-1">
                <?php include("formFrase.php"); ?>
            </div>
            <div id="tabs-2">
                <?php include("formProcurarFrase.php"); ?>
            </div>
...
<script type="text/javascript" src="/visao/recursos/js/jquery-1.10.2.min.js"></script>
        ...
        <script src="./recursos/js/ajax/Frase.js"></script>
        <script type="text/javascript" src="./recursos/js/Editor2.js"></script>
