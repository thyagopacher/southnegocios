...
if(isset($_SESSION["nome"])){
    ...
    $conexao = new Conexao();
    $codachado = filter_input(INPUT_GET, 'codachado', FILTER_VALIDATE_INT);
    if (isset($codachado)) {
        $achado = $conexao->comandoArray("select * from achado where codachado = '{$codachado}'");
        $titulo = "Alterar achados e perdidos";
        $action = "../control/AtualizarAchado.php";
    } else {
        $titulo = "Cadastrar achados e perdidos";
        $action = "../control/InserirAchado.php";
    }
    $nivelp = $conexao->comandoArray("select * from nivelpagina where codnivel = '{$_SESSION["codnivel"]}' and codpagina = '26'");
}
?>
<!DOCTYPE html>
<html lang="pt-br">
    ...
    <body>
        ...
        <h3><?= $titulo ?></h3>
         <div id="tabs" style="width: 955px; margin: 0 auto;  margin-top: 55px;">
            ...
            <?php if($_SESSION["codnivel"] == 1){?>
            <div id="tabs-3">
                <?php include("formStatusAchado.php"); ?>
            </div>
            <?php }?>
            <div id="tabs-4">
                <?php include("formTipoAchado.php"); ?>
            </div>
...
<script type="text/javascript" src="/visao/recursos/js/jquery-1.10.2.min.js"></script>
        ...
        <script src="./recursos/js/ajax/AchadoPerdido.js"></script>
        <script src="./recursos/js/ajax/TipoAchado.js"></script>
        <script src="./recursos/js/ajax/StatusAchado.js"></script>
