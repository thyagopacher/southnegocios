<form id="fpcomunicado" action="../control/ProcurarComunicadoRelatorio.php" target="_blank" role="form" class="form-horizontal form-groups-bordered" method="POST" onsubmit="return false;">                       
    ...
    <button onclick="procurarComunicado(false)">Procurar</button>
    <?php
    if(isset($nivelp["gerapdf"]) && $nivelp["gerapdf"] == 1){
        echo '<button onclick="abreRelatorioComunicado()">Gera PDF</button> ';
    }
    if(isset($nivelp["geraexcel"]) && $nivelp["geraexcel"] == 1){
        echo '<button onclick="abreRelatorioComunicado2()">Gera Excel</button>';
    }
    ?>
</form>
<?php include("./carregando.php");?>
<div id="listagemComunicado"></div>
