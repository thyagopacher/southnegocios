...
if (!isset($pessoa->nome) || $pessoa->nome == NULL || $pessoa->nome == "") {
    ...
    if ($sit_retorno) {
        ...
        //procurando se existe a carteira cliente manual
        $carteirap = $conexao->comandoArray("select * from carteira where nome = 'CLIENTE MANUAL' and codempresa = '{$_SESSION['codempresa']}'");
        if(!isset($carteirap) || $carteirap["codcateira"] == NULL || $carteirap["codcarteira"] == ""){
        ...
        $importacaop = $conexao->comandoArray("select * from importacao where codcarteira = '{$codigo_carteira}' and codempresa = '{$_SESSION['codempresa']}'");
        if(!isset($importacaop) || $importacaop["codimportacao"] == NULL || $importacaop["codimportacao"] == ""){
            ...
            $importacao->qtdimportado   = 1;
            $importacao->qtdnimportado  = 0;
            $importacao->inserir();
        }else{
            $importacao                 = new Importacao($conexao);
            $importacao->codimportacao  = $importacao["codimportacao"];
            $importacao->codcarteira    = $codigo_carteira;
            $importacao->codempresa     = $_SESSION['codempresa'];
            $importacao->codfuncionario = $_SESSION['codpessoa'];
            $importacao->data           = date('Y-m-d');
            $importacao->qtdimportado   = $importacaop["qtdimportado"] + 1;
            $importacao->qtdnimportado  = 0;
            $importacao->atualizar();            
        }
        ...
        if ($res === FALSE) {
            ...
            if(isset($_POST["dtagenda"]) && $_POST["dtagenda"] != NULL && $_POST["dtagenda"] != ""){
                ...
                $sql = "select * from agenda where codempresa = '{$agenda->codempresa}' and atendido = 'n' and codfuncionario = '{$agenda->codfuncionario}' and codpessoa = '{$agenda->codpessoa}'";    
