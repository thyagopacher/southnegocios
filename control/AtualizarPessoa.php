        ...
        if ($res === FALSE) {
            ...
            if(isset($_POST["telefone"]) && $_POST["telefone"] != NULL && count($_POST["telefone"]) > 0){
                ...
                foreach ($_POST["telefone"] as $key => $telefone2) {
                    ...
                    $telefone->codempresa = $codempresa;
                    $telefone->codfuncionario = $_SESSION['codpessoa'];
                    $telefone->codpessoa = $_POST["codpessoa"];
                     $telefone->codtipo = $_POST["tipotelefone"][$key];
//                    echo "Tipo: {$telefone->codtipo}";
                    $telefone->dtcadastro = date("Y-m-d H:i:s");
                    $telefone->numero = $telefone2;
                    $resInserir = $conexao->comando("INSERT INTO `telefone`(`numero`, `dtcadastro`, `codfuncionario`, `codpessoa`, `codtipo`, `codempresa`)
                    VALUES ('{$telefone->numero}', '".date("Y-m-d H:i:s")."', '{$telefone->codfuncionario}', '{$telefone->codpessoa}', '{$telefone->codtipo}', '{$telefone->codempresa}')");
                    if($resInserir == FALSE){
...
if ($sit_retorno) {
    ...
    $atendimento->codempresa     = $pessoa->codempresa;
