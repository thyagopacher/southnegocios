...
if (isset($_POST["codempresa"]) && $_POST["codempresa"] != NULL && $_POST["codempresa"] != "") {
    $and .= " and b2.codempresa = {$_POST["codempresa"]}";
}
...
$sql = "select distinct pessoa.codpessoa, pessoa.codempresa, pessoa.nome, pessoa.imagem
    ...
    where 1 = 1
    and b2.codempresa = {$_SESSION["codempresa"]}
...
echo '<p style="font-size: 12px; color: #666;" >Encontrou ', $qtd, ' resultados<p>';
...
echo '<th style="font-size: 12px; color: #666;" class="sorting">Unidade</th>';
echo '<th style="font-size: 12px; color: #666;" class="sorting">Colaborador</th>';
echo '<th style="font-size: 12px; color: #666;" class="sorting">Valor</th>';
echo '</tr>';
echo '</thead>';
echo '<tbody>';
for ($i = 0; $i < $qtd; $i++) {
    ...
    if ((!isset($_POST["procurar"]) || $_POST["procurar"] == "0") && $ranking[$i]["codfuncionario"] != $_SESSION['codpessoa']) {
        echo '<td style="text-align: left;">--</td>';
    } else {
